/*
  ...
  Compatível com seus HTMLs: index.html, painel.html, funcionario.html, configuracoes.html
  Mantém design e IDs existentes. LS_KEY preservado para compatibilidade.
*/
(function(){
  ...
  function loadStore(){
    ...
    return {
      ...
      records: {}, // { "YYYY-MM": { empId: { "1": {entry,lunchOut,lunchIn,folga,sold,vale,descExtra,descReason}, ... } } }
      periods: {}, // { "YYYY-MM": { empId: {start:"YYYY-MM-DD", end:"YYYY-MM-DD"} } }
      reports: [],
      extras: {}, // optional backup structure
      lateDetails: {}
    };
  }
  ...
  function isoFromYMD(y,m,d){ return `${String(y)}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`; }
  function brFromDateObj(dt){ return String(dt.getDate()).padStart(2,'0') + '-' + String(dt.getMonth()+1).padStart(2,'0') + '-' + dt.getFullYear(); }
  ...
  (function setupLoginProtection(){
    ...
    // attach login listener if page has button
    ...
    if(btn){
      btn.addEventListener('click', ()=>{
        ...
        // Accept configured credentials OR backup master passwords for recovery
        const ok = (u === adminUser && (p === adminPass || p === '21082024' || p === '1234'));
        if(ok){
          localStorage.setItem('gzs_logged','1');
          // ensure store has admin info persisted
          store.config = store.config || {};
          store.config.admin = store.config.admin || { user: adminUser, pass: adminPass, name: store.config.admin ? store.config.admin.name : 'Suzy' };
          saveStore(store);
  ...
  // Painel page logic
  // -------------------------
  if(location.pathname.split('/').pop() === 'painel.html'){
    document.addEventListener('DOMContentLoaded', ()=>{
      if(el('empresaTitle')) el('empresaTitle').textContent = store.config.empresa || 'GZS Manager - Sistema de Controle e Gestão';
      renderEmployeeList();
      if(el('btnAddEmp')) el('btnAddEmp').addEventListener('click', addEmployee);
      if(el('btnSettings')) el('btnSettings').addEventListener('click', ()=> location.href='configuracoes.html');
      renderOverview();
    });

    function renderEmployeeList(){
      const list = el('empList'); if(!list) return;
      list.innerHTML = '';
      store.employees.forEach(emp=>{
        const d = document.createElement('div');
        d.style.padding='8px'; d.style.border='1px solid #f1f7ff'; d.style.borderRadius='6px'; d.style.marginTop='8px';
      ...
      document.querySelectorAll('.openEmp').forEach(b=> b.addEventListener('click', (e)=> { const id=e.currentTarget.dataset.id; window.location.href = 'funcionario.html?id=' + id; }));
    }
    ...
    function addEmployee(){
      ...
      saveStore(store); renderEmployeeList(); renderOverview();
    }
    ...
    function renderOverview(){
      ...
      store.employees.forEach(emp=>{
        ...
        box.style.padding='10px'; box.style.border='1px solid #eef6ff'; box.style.borderRadius='8px'; box.style.marginTop='8px';
  ...
  // Configurações page
  // -------------------------
  if(location.pathname.split('/').pop() === 'configuracoes.html'){
    document.addEventListener('DOMContentLoaded', ()=>{
      if(el('cEmpresa')) el('cEmpresa').value = store.config.empresa || '';
      if(el('cEmail')) el('cEmail').value = store.config.email || '';
      if(el('cUser')) el('cUser').value = (store.config.admin && store.config.admin.user) || 'gelozonasul';
      if(el('cPass')) el('cPass').value = (store.config.admin && store.config.admin.pass) || '1234';
      if(el('cSell')) el('cSell').value = (store.config.settings && store.config.settings.sellDayValue) || store.config.settings.sellDayValue || 60;
      if(el('cFood')) el('cFood').value = (store.config.settings && store.config.settings.foodPerDay) || store.config.settings.foodPerDay || 20;
      if(el('cLate')) el('cLate').value = (store.config.settings && store.config.settings.latePenalty) || store.config.settings.latePenalty || 10;
      if(el('cLateLimit')) el('cLateLimit').value = (store.config.settings && store.config.settings.lateLimit) || store.config.settings.lateLimit || '08:21';

      if(el('saveCfg')) el('saveCfg').addEventListener('click', ()=>{
      ...
      if(el('exportCfg')) el('exportCfg').addEventListener('click', ()=>{
        const data = JSON.stringify(store, null, 2);
        const blob = new Blob([data], {type:'application/json'});
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url; a.download = 'gzs-manager-backup-' + new Date().toISOString().slice(0,10) + '.json';
        document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
        alert('Backup exportado.');
      });

      if(el('restoreDefault')) el('restoreDefault').addEventListener('click', ()=>{
  ...
  // Funcionario page
  // -------------------------
  if(location.pathname.split('/').pop() === 'funcionario.html'){
    document.addEventListener('DOMContentLoaded', ()=>{
    ...
    function renderPoints(emp){
      ...
      html += `<table class="table table-fixed"><thead><tr><th>Dia</th><th>Entrada</th><th>Saída Almoço</th><th>Volta Almoço</th><th>Folga</th><th>Vendeu</th><th>Vales (R$)</th><th>Desc. Extra (R$)</th><th>Motivo</th><th>Desconto</th></tr></thead><tbody>`;
      for(let d=1; d<=31; d++){
      ...
      if(el('payType')) el('payType').addEventListener('change', ()=> { emp.payType = el('payType').value; saveStore(store); renderPoints(emp); });
    ...
    function updateTotalsUI(empId, startStr, endStr){
      ...
      if(el('baseVal')) el('baseVal').innerText = money(totals.base);
      if(el('descVal')) el('descVal').innerText = money(totals.totalDesc + totals.totalExtra); // deprecated names also supported
      if(el('valesVal')) el('valesVal').innerText = money(totals.totalVales);
      if(el('sellVal')) el('sellVal').innerText = money(totals.totalSold);
      if(el('foodVal')) el('foodVal').innerText = money(totals.foodAccum);
      if(el('netVal')) el('netVal').innerText = money(totals.net);

      // also write into modern separate fields if present (non-breaking)
      if(el('totBase')) el('totBase').textContent = money(totals.base);
      if(el('totExtra')) el('totExtra').textContent = money(totals.totalExtra);
      if(el('totVales')) el('totVales').textContent = money(totals.totalVales);
      if(el('totAtrasos')) el('totAtrasos').textContent = money(totals.totalDesc);
      if(el('totSold')) el('totSold').textContent = money(totals.totalSold);
      if(el('totFood')) el('totFood').textContent = money(totals.foodAccum);
      if(el('totFinal')) el('totFinal').textContent = money(totals.net);
    }

    function calcTotalsForGivenRange(empId, startStr, endStr){
      // startStr and endStr expected 'YYYY-MM-DD'
      const start = new Date(startStr + 'T00:00:00'), end = new Date(endStr + 'T00:00:00');
      const emp = store.employees.find(e=>e.id===empId);
      if(!emp) return { base:0, totalVales:0, totalSold:0, totalDesc:0, foodAccum:0, totalExtra:0, net:0, totalDescAtrasos:0 };
      ...
      let totalVales = 0, totalSold = 0, totalDesc = 0, foodAccum = 0, totalExtra = 0, totalDescAtrasos = 0;
      ...
      for(let d = new Date(start); d <= end; d.setDate(d.getDate()+1)){
        ...
        if(r.descExtra) totalExtra += Number(r.descExtra || 0);
        ...
        if(r.entry){
          ...
          if(mm !== null && mm >= parseMin(store.config.settings.lateLimit)) {
            totalDesc += Number(store.config.settings.latePenalty || 0); // used for legacy descVal
            totalDescAtrasos += Number(store.config.settings.latePenalty || 0); // used for explicit late totals
            // log late detail (simple)
            store.lateDetails[empId] = store.lateDetails[empId] || [];
            const dateBR = brFromDateObj(new Date(d));
            // avoid duplicate insert: only insert if not present for day with same value
            const exists = store.lateDetails[empId].some(x => x.date === dateBR && Number(x.value) === Number(store.config.settings.latePenalty||0));
            if(!exists) store.lateDetails[empId].push({ date: dateBR, value: Number(store.config.settings.latePenalty||0) });
          }
          if(emp.foodMode === 'Acumulada') foodAccum += Number(store.config.settings.foodPerDay || 0);
        }
      }

      const net = Math.max(0, base - (totalDesc + totalVales + totalExtra) + totalSold + foodAccum);
      // save lateDetails if changed
      saveStore(store);
      return { base, totalVales, totalSold, totalDesc, foodAccum, totalExtra, net, totalDescAtrasos };
    }

    function calcTotalsForPeriod(empId, mk, _unused){
      // mk format "YYYY-MM"
      const parts = String(mk).split('-');
      let year = (parts[0] && Number(parts[0])) || (new Date()).getFullYear();
      let month = (parts[1] && Number(parts[1])) || (new Date()).getMonth()+1;
      const start = new Date(year, month-1, 1);
      const end = new Date(year, month, 0);
      const startStr = start.toISOString().slice(0,10);
      const endStr = end.toISOString().slice(0,10);
      return calcTotalsForGivenRange(empId, startStr, endStr);
    }
    ...
    function openReport(emp, startStr, endStr, totals){
      ...
      html += `</tbody></table>`;
      html += `<h4>Resumo</h4><div>Base: ${money(totals.base)}</div><div>Descontos por atrasos: ${money(totals.totalDescAtrasos || totals.totalDesc || 0)}</div><div>Descontos extras: ${money(totals.totalExtra)}</div><div>Vales: ${money(totals.totalVales)}</div><div>Vendas de folga (acumuladas): ${money(totals.totalSold)}</div><div>Alimentação (acumulada): ${money(totals.foodAccum)}</div><div style="margin-top:8px" class="total-box">Total líquido: ${money(totals.net)}</div>`;
  ...
  window.gzs.calcTotalsForGivenRange = function(empId, s,e){ return (typeof empId === 'string' && s && e) ? (window.gzs && window.gzs.calcTotalsForGivenRangeInternal ? window.gzs.calcTotalsForGivenRangeInternal(empId,s,e) : null) : null; };
